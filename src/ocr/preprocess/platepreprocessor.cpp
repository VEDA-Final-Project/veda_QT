...
cv::Rect estimatePlateContentRect(const cv::Mat &gray) {
  ...
  const cv::Mat base =
      stretched.empty() ? (clahe.empty() ? gray : clahe) : stretched;
  ...
  int bottom = -1;
  const bool hasCols =
      findProjectionRange(smoothCols, colThreshold, borderX,
                          gray.cols - borderX - 1, &left, &right);
  const bool hasRows =
      findProjectionRange(smoothRows, rowThreshold, borderY,
                          gray.rows - borderY - 1, &top, &bottom);

  if (hasCols && hasRows) {
    const cv::Rect projectedRect(left, top, (right - left) + 1,
                                 (bottom - top) + 1);
    if (projectedRect.width >=
            static_cast<int>(std::lround(gray.cols * 0.45)) &&
        projectedRect.height >=
            static_cast<int>(std::lround(gray.rows * 0.28))) {
      const int padX =
          std::max(3, static_cast<int>(std::lround(
                          projectedRect.width * (lowQuality ? 0.09 : 0.08))));
      ...
      const cv::Rect tightExpanded =
          clampRectToImage(cv::Rect(projectedRect.x - std::max(2, padX / 2),
                                    projectedRect.y - std::max(2, padY / 2),
                                    projectedRect.width + std::max(4, padX),
                                    projectedRect.height + std::max(4, padY)),
                           gray.size());
      if (!tightExpanded.empty() && (tightExpanded.width < (gray.cols - 2) ||
  ...
  const int padX =
      std::max(4, static_cast<int>(std::lround(contentRect.width *
                                               (lowQuality ? 0.16 : 0.10))));
  const int padY =
      std::max(3, static_cast<int>(std::lround(contentRect.height *
...
cv::Rect fallbackInnerTrimRect(const cv::Mat &gray) {
  ...
  return clampRectToImage(cv::Rect(padX, padTop,
                                   std::max(1, gray.cols - (padX * 2)),
...
bool detectPlateContour(const cv::Mat &gray,
...
orderQuadPoints(const std::vector<cv::Point2f> &points) {
...
cv::Mat warpPlatePerspective(const cv::Mat &rgb,
  ...
  const int targetW =
      std::clamp(static_cast<int>(std::lround(targetH * aspect)), kMinWarpWidth,
                 maxTargetW);
...
bool preprocessPlateImage(const QImage &image, const int inputWidth,
  ...
  *resultOut = PlatePreprocessResult{};
  ...
  if (extractDeskewQuadFromCrop(gray, &quad) && validateQuadGeometry(quad) &&
    ...
    const cv::Mat warpedRgb = warpPlatePerspective(
        resultOut->normalizedRgb, quad, inputWidth, inputHeight);
    if (!warpedRgb.empty()) {
