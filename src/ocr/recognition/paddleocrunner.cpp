...
namespace {
constexpr int kInvalidScore =
    std::numeric_limits<int>::min() / 4; // int가 가질수 있는 가장 작은 값 /4
...
bool resolveTimeAndClassDims(const std::vector<int64_t> &shape,
  ...
  *classCountOut = 0;
  ...
  auto valid = [&elementCount](const int t, const int c) {
    if (t <= 0 || c <= 1) {
      return false;
    }
...
cv::Mat ensureRgbImage(const cv::Mat &image) {
  ...
  if (image.type() == CV_8UC3) // 8비트 unsigned, 3채널 이미지
  {
    return image;
  }

  if (image.type() == CV_8UC1) // 8비트 unsigned, 1채널 이미지 (그레이스케일)
  {
    cv::Mat rgb;
  ...
  if (image.type() == CV_8UC4) // 8비트 unsigned, 4채널 이미지 (예: RGBA)
  {
    cv::Mat rgb;
...
bool PaddleOcrRunner::init(const QString &modelPath, const QString &dictPath,
  ...
  if (!modelInfo.exists() || !modelInfo.isFile()) {
    if (errorOut) {
      *errorOut =
          QStringLiteral("ONNX model file not found: %1").arg(modelPath);
    }
    return false;
  }
...
bool PaddleOcrRunner::isReady() const { return m_ready; }

postprocess::OcrCandidate
PaddleOcrRunner::runSingleCandidate(const cv::Mat &imageRgb) {
  if (imageRgb.empty()) {
    return postprocess::OcrCandidate{};
  }

  return runOcrOnImage(imageRgb);
}

bool PaddleOcrRunner::loadDictionary(const QString &dictPath,
                                     QString *errorOut) {
  m_dictionary.clear();
...
std::vector<QString> PaddleOcrRunner::fallbackDictionary() {
  const QString chars = QStringLiteral("0123456789"
                                       "가나다라마바사아자하"
                                       "거너더러머버서어저허"
                                       "고노도로모보소오조호"
                                       "구누두루무부수우주");
...
postprocess::OcrCandidate
PaddleOcrRunner::runOcrOnImage(const cv::Mat &imageRgb) {
  ...
  try {
    outputTensors = m_session->Run(Ort::RunOptions{nullptr},
...
cv::Mat PaddleOcrRunner::makeInputBlob(const cv::Mat &imageRgb) const {
  ...
  return cv::dnn::blobFromImage(
      canvas, 1.0 / 127.5, cv::Size(m_inputWidth, m_inputHeight),
      cv::Scalar(127.5, 127.5, 127.5), false, false, CV_32F);
}
...
QString PaddleOcrRunner::decodeCtcGreedy(const cv::Mat &scores,
  ...
  if (confidenceOut && confCount > 0) {
    *confidenceOut =
        static_cast<int>(std::lround((confSum / confCount) * 100.0));
  }
