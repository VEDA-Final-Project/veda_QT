...
QByteArray randomBytes(int size) {
  ...
  for (int i = 0; i < size; ++i) {
    bytes[i] =
        static_cast<char>(QRandomGenerator::system()->generate() & 0xFF);
  }
  return bytes;
}
} // namespace
...
QByteArray DataProtection::loadOrCreateDataKey() {
  ...
  if (keyFile.exists() && keyFile.open(QIODevice::ReadOnly)) {
    const QByteArray protectedKey =
        QByteArray::fromBase64(keyFile.readAll().trimmed());
    keyFile.close();
  ...
  const QByteArray newKey = randomBytes(kAes256KeySize);
...
QString DataProtection::keyFilePath() const {
  const QString appConfigDir = QDir(QCoreApplication::applicationDirPath())
...
QByteArray DataProtection::protectKeyForStorage(const QByteArray &key) const {
  ...
  if (!CryptProtectData(&inputBlob, L"VEDA Data Key", nullptr, nullptr, nullptr,
...
QString DataProtection::encryptWithAesGcm(const QString &plainText,
  ...
  if (BCryptOpenAlgorithmProvider(&algHandle, BCRYPT_AES_ALGORITHM, nullptr,
                                  0) != 0) {
    return plainText;
  }
  ...
  const QByteArray nonce = randomBytes(kGcmNonceSize);
  ...
  const NTSTATUS status = BCryptEncrypt(
      keyHandle, reinterpret_cast<PUCHAR>(const_cast<char *>(plainUtf8.data())),
...
QString DataProtection::decryptWithAesGcm(const QString &storedValue,
  ...
  if (status == 0) {
    result =
        QString::fromUtf8(plainText.constData(), static_cast<int>(plainSize));
  }
