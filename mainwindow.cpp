...
MainWindow::~MainWindow() {
  ...
  // m_ocrManager uses QObject parent, so it cleans up automatically

  // OCR 작업이 진행 중이라면 완료될 때까지 대기 (크래시 방지)
...
void MainWindow::decreaseSync() {
  m_syncDelayMs -= 100;
  // 음수 딜레이는 사실상 0과 같음 (미래 데이터를 보여줄 순 없으니, 즉시
  // 보여주기)
  if (m_syncDelayMs < 0)
    m_syncDelayMs = 0;
  updateSyncLabel();
...
void MainWindow::updateFrame(const QImage &frame) {
  ...
  for (const ObjectInfo &obj : m_currentObjects) {
    // XML 데이터 분석 결과 좌표 범위가 0~1000 이 아니라 큰 값(예: 1986.0)으로
    // 옴. 카메라 해상도에 맞춘 픽셀 좌표로 추정됨. 임시로 5MP (2592 x 1944)
    // 또는 4K (3840 x 2160) 중 하나로 가정 후 비율 계산. 현재 bottom이 1986인
    // 것으로 보아 높이가 약 1944(5MP) 또는 2160(4K)일 가능성 높음. 안전하게
    // 비율계산을 위해 SOURCE_WIDTH/HEIGHT 상수를 정의하여 사용 권장.
    ...
    const float CROP_OFFSET_X =
        (SOURCE_TOTAL_WIDTH - EFFECTIVE_WIDTH) / 2.0f; // (3840-2880)/2 = 480

    // 좌표 보정: (원본X - 오프셋) / 유효폭 * 화면폭
    int x =
        ((obj.rect.x() - CROP_OFFSET_X) / EFFECTIVE_WIDTH) * keyFrame.width();
    ...
    // 잘려나간 영역의 객체는 안 그리거나 클리핑됨 (Qt Painter가 알아서
    // 클리핑함)
    ...
    // --- OCR Processing ---
    // --- OCR Processing ---
    // Vehicle(차량)은 제외하고, 정확한 LicensePlate(번호판) 객체만 OCR 수행
    // Vehicle(차량)은 제외하고, 정확한 LicensePlate(번호판) 객체만 OCR 수행
    if (obj.type == "LicensePlate") {
      // 1. ROI 추출 (Bounding Box)
      QRect safeRect = rect.intersected(keyFrame.rect());
      if (!safeRect.isEmpty()) {
        // 이미 수행 중인 OCR 작업이 없을 때만 수행 (스킵)
        if (!m_ocrWatcher.isRunning()) {
          QImage roi = keyFrame.copy(safeRect); // ROI 복사 (Deep Copy)
          int currentObjId = obj.id;
          // QString currentObjType = obj.type;

          m_processingOcrId = currentObjId; // ID 저장

          // 비동기 실행
          QFuture<QString> future = QtConcurrent::run([this, roi]() {
            // 이 람다 함수는 별도 스레드에서 실행됨
            // Qt는 QImage Deep Copy를 스레드로 안전하게 전달함
  ...
  m_videoLabel->setPixmap(QPixmap::fromImage(keyFrame).scaled(
